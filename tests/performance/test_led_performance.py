...
class TestHSVLookupTablePerformance:
    ...
    def test_hsv_lut_lookup_performance(self):
        ...
        # REQUIREMENT: LUT must be faster than reference
        speedup = stats_ref['avg_ms'] / stats_lut['avg_ms']
        assert speedup > 1.5, \
            f"HSV LUT not faster enough: {speedup:.1f}× (target: >1.5×)"
...
class TestFrameTimingPerformance:
    ...
    def test_precision_timer_accuracy(self):
        """Verify timer maintains 50Hz ±1%."""
        from firmware.src.animation.timing import AnimationPlayer, AnimationSequence, Keyframe
        ...
        # REQUIREMENT: Average frame time within ±1% of target
        error_pct = abs(avg_frame_time - target_frame_time) / target_frame_time * 100
        assert error_pct < 1.0, \
            f"Frame timing error too high: {error_pct:.2f}% (target: <1%)"
    ...
    def test_frame_jitter_measurement(self):
        """Verify jitter is <1ms."""
        from firmware.src.animation.timing import AnimationPlayer, AnimationSequence
        ...
        # REQUIREMENT: Average jitter <1ms
        assert avg_jitter < 1.0, \
            f"Average jitter too high: {avg_jitter:.3f}ms (target: <1ms)"

        # REQUIREMENT: Max jitter <5ms (allows for occasional OS interrupts)
        assert max_jitter < 5.0, \
            f"Max jitter too high: {max_jitter:.3f}ms (target: <5ms)"
...
class TestPatternPerformance:
    ...
    def test_breathing_pattern_performance(self):
        """Verify breathing pattern renders in <1ms."""
        ...
        # REQUIREMENT: <1ms per frame for breathing
        assert stats['avg_ms'] < 1.0, \
            f"Breathing pattern too slow: {stats['avg_ms']:.3f}ms (target: <1ms)"

        print(f"\n  Breathing pattern: {stats['avg_ms']:.4f}ms/frame (p95: {stats['p95_ms']:.4f}ms)")

    def test_pulse_pattern_performance(self):
        """Verify pulse pattern renders in <1ms."""
        ...
        for _ in range(1000):
            profiler.start()
            pattern.update((255, 100, 50))
            profiler.stop()

        stats = profiler.get_stats()

        # REQUIREMENT: <1ms per frame for pulse
        assert stats['avg_ms'] < 1.0, \
            f"Pulse pattern too slow: {stats['avg_ms']:.3f}ms (target: <1ms)"

        print(f"\n  Pulse pattern: {stats['avg_ms']:.4f}ms/frame (p95: {stats['p95_ms']:.4f}ms)")

    def test_spin_pattern_performance(self):
        """Verify spin pattern renders in <1ms."""
        ...
        for _ in range(1000):
            profiler.start()
            pattern.update((200, 200, 255))
            profiler.stop()

        stats = profiler.get_stats()

        # REQUIREMENT: <1ms per frame for spin
        assert stats['avg_ms'] < 1.0, \
            f"Spin pattern too slow: {stats['avg_ms']:.3f}ms (target: <1ms)"
...
class TestScalabilityStress:
    ...
    def test_1000_led_scalability(self):
        ...
        # REQUIREMENT: Must still complete in <20ms for 50Hz
        assert stats['avg_ms'] < 20.0, \
            f"1000 LED render too slow: {stats['avg_ms']:.3f}ms (target: <20ms)"

        print(f"\n  1000 LED breathing: {stats['avg_ms']:.3f}ms/frame")

    def test_1000hz_frame_rate_stress(self):
        """Verify system can theoretically support 1000Hz frame rate."""
        from firmware.src.led.patterns.breathing import BreathingPattern
        ...
        # REQUIREMENT: Must complete in <1ms for 1000Hz
        assert stats['avg_ms'] < 1.0, \
            f"Pattern render too slow for 1000Hz: {stats['avg_ms']:.4f}ms (target: <1ms)"
...
class TestAlgorithmicComplexity:
    ...
    def test_breathing_pattern_complexity(self):
        ...
        for i in range(len(led_counts) - 1):
            ...
            # Allow ±30% variance (overhead, caching effects)
            assert 0.7 * ratio_leds < ratio_time < 1.3 * ratio_leds, \
    ...
    def test_easing_function_complexity(self):
        ...
        from firmware.src.animation.easing import ease
        ...
        # REQUIREMENT: Easing function variance <10% (proves O(1))
        variance_pct = (stats['std_ms'] / stats['avg_ms']) * 100
        assert variance_pct < 10, \
            f"Easing function not O(1): {variance_pct:.1f}% variance (target: <10%)"
...
class TestIntegrationPerformance:
    ...
    def test_full_animation_sequence_performance(self):
        """Verify complete animation sequence maintains 50Hz."""
        from firmware.src.animation.timing import AnimationPlayer, AnimationSequence
        ...
        # REQUIREMENT: Full animation maintains <20ms frame time
        assert stats['avg_ms'] < 20.0, \
            f"Full animation too slow: {stats['avg_ms']:.3f}ms (target: <20ms)"

        # REQUIREMENT: P95 latency <22ms (allows for occasional spikes)
        assert stats['p95_ms'] < 22.0, \
            f"P95 latency too high: {stats['p95_ms']:.3f}ms (target: <22ms)"
